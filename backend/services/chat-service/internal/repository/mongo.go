	...
	type Chat struct {
		...
		Members     []string  `bson:"members" json:"members"` // store only user IDs
	...
	func (r *Repository) ListChatsForUser(ctx context.Context, userID string, limit int64) ([]*Chat, error) {
		filter := bson.M{"members.id": userID} // filter by Member.ID
	...
	func (r *Repository) UpdateChat(ctx context.Context, chat *Chat) error {
		...
		_, err := r.coll.UpdateByID(ctx, chat.ID, bson.M{
			"$set": bson.M{
